...
class NDRrequests:
    ...
    token_request = "https://login.microsoftonline.com/"+api_info['tenant id']+"/oauth2/v2.0/token" 
    ...
    def headers(self):
        if not self.access_token:
            print("access token does not exist, either server is down or init didn't work")
        else:
    ...
    def _return_response(self, url:str, headers:dict=None)->Union[Dict[str, Any], None]:
        if headers is None:
        ...
        if response.status_code == 200:
            ...
            print(f"failed to get response from server - got status code:{response.status_code} and json:{response.json()}")
            return None
    ...
    def create_url(self, list_id: str)->None:
        try: 
            ...
            self.current_url += "https://graph.microsoft.com/v1.0/sites/"+NDRrequests.api_info["site id"]+"/lists/"
    ...
    def get_key_names(self, list_id: str)->pd.DataFrame:
        ...
        if self.current_url is None:
            ...
            response = [[i["name"], i["displayName"], i["description"]] for i in unparsed_response["value"][1:]]
        if response is not None:
            result = pd.DataFrame(response[1:], columns=["name", "displayName", "description"])
        else:
    ...
    def get_LAS_by_key(self, *, key:str='survid', value:str=None)->Union[Dict[str, Any], None]:
        ...
        if response is not None:
            result = {val["fields"][key]+f" {i}":val["fields"]["fnam"] for i, val in enumerate(response["value"])}
        return result
    ...
    def get_LAS_by_quadrant(self, quadrant_value: str)->Union[Dict[str, Any], None]:
        ...
        if response is not None:
            result = {f"quadrant well {i}": val["fields"]["survid"] for i, val in enumerate(response["value"])}
        return result
...
class BGSTopsScraper:
    ...
    def create_ids_url(self)->list:
        doc = self.get_well_info(BGSTopsScraper.links)
        self._well_ids_url = {i.get_text(): BGSTopsScraper.base_url+i.get("href") for i in doc.find_all("a")}
    ...
    def get_pandas_from_well_id(self, well_id):
        ...
        if well_id not in self.well_ids:
            ...
            model = df[["Top Down Hole Depth"]].apply(lambda x: np.round(f2m*x, 1)) # convert feet to meters
